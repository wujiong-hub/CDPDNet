...
def get_loader_with_text(args):
    ...
    data_dicts_test = [{'image': image, 'label': label, 'post_label': post_label, 'name': name, 'prompt':prompt}
                for image, label, post_label, name, prompt in zip(test_img, test_lbl, test_post_lbl, test_name, test_prompt)]
    print('test len {}'.format(len(data_dicts_test)))
...
class getTrainSamples(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(path, 'r') as hf:
            x = hf['image'][:]
            y = hf['post_label'][:]
            name = hf['name'][:].astype(str)  # Convert back to regular strings if needed

        #data_dicts_train = {'image': x, 'post_label': y, 'name': name}
    ...
    def collate_fn(batch):
        ...
        images, post_labels, names = tuple(zip(*batch))
        images = torch.stack(images, dim=0)
        post_labels = torch.stack(post_labels, dim=0)
        #return images, labels, names
        b, c, *dims = images.shape
        images = images.reshape(b * c, *dims)
        b, c, *dims = post_labels.shape
        post_labels = post_labels.reshape(b * c, *dims)

        #print("-------:", images.shape, post_labels.shape, names)
        
        data_dicts_train = {'image': images, 'post_label': post_labels, 'name': np.concatenate(names).tolist()}
        #print(images[0,...].shape, post_labels[0,...].shape, names)
        return data_dicts_train


def get_loader_trainsamples(args, train_samples):
    val_transforms = Compose(
        [
            LoadImageh5d(keys=["image", "label"]),
            AddChanneld(keys=["image", "label"]),
    ...
    if args.phase == 'train':
        #train_samples = sorted(glob.glob('./out/samples/*'))[0:440400]
        train_dataset = getTrainSamples(train_samples)
        train_sampler = torchDistributedSampler(train_dataset, shuffle=True)
...
def get_loader_mots(args):
    ...
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item +'_train.txt'):
            ...
            train_img.append(args.data_root_path + line.strip().split()[0])
            train_lbl.append(args.data_root_path + line.strip().split()[1])
            #print(args.data_root_path + name.replace('label', 'post_mots_label') + '.h5')
            train_post_lbl.append(args.data_root_path + name.replace('label', 'post_mots_label') + '.h5')
            train_name.append(name)
            #print(args.data_root_path + name.replace('label', 'post_mots_label') + '.h5', name)
    data_dicts_train = [{'image': image, 'label': label, 'post_label': post_label, 'name': name}
                for image, label, post_label, name in zip(train_img, train_lbl, train_post_lbl, train_name)]
    print('train len {}'.format(len(data_dicts_train)))
    ...
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item +'_val.txt'):
            ...
            val_img.append(args.data_root_path + line.strip().split()[0])
            val_lbl.append(args.data_root_path + line.strip().split()[1])
            val_post_lbl.append(args.data_root_path + name.replace('label', 'post_mots_label') + '.h5')
            val_name.append(name)
    data_dicts_val = [{'image': image, 'label': label, 'post_label': post_label, 'name': name}
                for image, label, post_label, name in zip(val_img, val_lbl, val_post_lbl, val_name)]
    print('val len {}'.format(len(data_dicts_val)))
    ...
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item +'_test.txt'):
            ...
            test_img.append(args.data_root_path + line.strip().split()[0])
            test_lbl.append(args.data_root_path + line.strip().split()[1])
            test_post_lbl.append(args.data_root_path + name.replace('label', 'post_mots_label') + '.h5')
            test_name.append(name)
    data_dicts_test = [{'image': image, 'label': label, 'post_label': post_label, 'name': name}
                for image, label, post_label, name in zip(test_img, test_lbl, test_post_lbl, test_name)]
    print('test len {}'.format(len(data_dicts_test)))
...
if __name__ == "__main__":
    train_loader, test_loader = get_loader_mots()
    for index, item in enumerate(train_loader):
        print(item['image'].shape, item['label'].shape, item['post_label'])
        #input()